...
import { BaseContract, Contract, ContractEventName, EventLog } from "ethers";
...
export async function paginatedEventQuery<T extends EventLog>(
  ...
  try {
    return (
      (
        ...
        .filter(
          (event: any) => event?.blockNumber >= searchConfig.fromBlock && event.blockNumber <= searchConfig.toBlock
        ) as Array<T>
    );
  } catch (error) {
